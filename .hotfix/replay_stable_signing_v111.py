...
content = re.sub(r"versionCode\s+\d+", "versionCode 4", content, count=1)
content = re.sub(r"versionName\s+['\"][^'\"]+['\"]", "versionName '1.1.1'", content, count=1)
build.write_text(content)

if "versionCode 4" not in content or "versionName '1.1.1'" not in content:
    raise SystemExit("v1.1.1 version configuration was not applied")
