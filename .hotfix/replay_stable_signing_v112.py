...
content = re.sub(r"versionCode\s+\d+", "versionCode 5", content, count=1)
content = re.sub(r"versionName\s+['\"][^'\"]+['\"]", "versionName '1.1.2'", content, count=1)
build.write_text(content)

if "versionCode 5" not in content or "versionName '1.1.2'" not in content:
    raise SystemExit("v1.1.2 version configuration was not applied")
